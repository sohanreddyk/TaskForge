...
class Task {
 public:
  ...
  Task(std::string uuid, std::string type, std::string payload,
       std::string state, uint64_t maxRetry, uint64_t retried,
...
void enqueue(redisContext *c, Task task, std::string queue, ScheduleOptions s) {
  ...
  if (s.type == ScheduleType::None) {
    redisCommand(c, "LPUSH cppq:%s:pending %s", queue.c_str(),
                 uuidToString(task.uuid).c_str());
    redisCommand(c,
                 "HSET cppq:%s:task:%s type %s payload %s state %s maxRetry %d "
                 "retried %d dequeuedAtMs %d",
                 queue.c_str(), uuidToString(task.uuid).c_str(),
                 task.type.c_str(), task.payload.c_str(),
                 stateToString(task.state).c_str(), task.maxRetry, task.retried,
                 task.dequeuedAtMs);
  } else if (s.type == ScheduleType::TimePoint) {
    redisCommand(c, "LPUSH cppq:%s:scheduled %s", queue.c_str(),
                 uuidToString(task.uuid).c_str());
    redisCommand(c,
                 "HSET cppq:%s:task:%s type %s payload %s state %s maxRetry %d "
...
std::optional<Task> dequeue(redisContext *c, std::string queue) {
  redisReply *reply = (redisReply *)redisCommand(
      c, "LRANGE cppq:%s:pending -1 -1", queue.c_str());
  ...
  redisCommand(c, "HGET cppq:%s:task:%s dequeuedAtMs", queue.c_str(),
               uuid.c_str());
  redisCommand(c, "HSET cppq:%s:task:%s dequeuedAtMs %lu", queue.c_str(),
               uuid.c_str(), dequeuedAtMs);
  redisCommand(c, "HSET cppq:%s:task:%s state %s", queue.c_str(), uuid.c_str(),
               stateToString(TaskState::Active).c_str());
  redisCommand(c, "LPUSH cppq:%s:active %s", queue.c_str(), uuid.c_str());
  reply = (redisReply *)redisCommand(c, "EXEC");

  if (reply->type != REDIS_REPLY_ARRAY || reply->elements != 10) return {};

  Task task = Task(uuid, reply->element[1]->str, reply->element[2]->str,
...
std::optional<Task> dequeueScheduled(redisContext *c, std::string queue,
                                     char *getScheduledScriptSHA) {
  redisReply *reply = (redisReply *)redisCommand(
      c, "EVALSHA %s 0 %s", getScheduledScriptSHA, queue.c_str());
  ...
  if (reply->type != REDIS_REPLY_ARRAY || reply->elements != 11) return {};
...
void taskRunner(redisOptions redisOpts, Task task, std::string queue) {
  ...
  try {
    ...
    task.retried++;
    redisCommand(c, "MULTI");
    redisCommand(c, "LREM cppq:%s:active 1 %s", queue.c_str(),
                 uuidToString(task.uuid).c_str());
    redisCommand(c, "HSET cppq:%s:task:%s retried %d", queue.c_str(),
                 uuidToString(task.uuid).c_str(), task.retried);
    if (task.retried >= task.maxRetry) {
      task.state = TaskState::Failed;
      redisCommand(c, "HSET cppq:%s:task:%s state %s", queue.c_str(),
                   uuidToString(task.uuid).c_str(),
                   stateToString(task.state).c_str());
      redisCommand(c, "LPUSH cppq:%s:failed %s", queue.c_str(),
                   uuidToString(task.uuid).c_str());
    } else {
      task.state = TaskState::Pending;
      redisCommand(c, "HSET cppq:%s:task:%s state %s", queue.c_str(),
  ...
  redisCommand(c, "HSET cppq:%s:task:%s state %s", queue.c_str(),
               uuidToString(task.uuid).c_str(),
               stateToString(task.state).c_str());
  redisCommand(c, "HSET cppq:%s:task:%s result %s", queue.c_str(),
               uuidToString(task.uuid).c_str(), task.result.c_str());
  redisCommand(c, "LPUSH cppq:%s:completed %s", queue.c_str(),
               uuidToString(task.uuid).c_str());
  redisCommand(c, "EXEC");
  redisFree(c);
}
      ...
      for (int i = 0; i < reply->elements; i++) {
        ...
        if (dequeuedAtMs + timeoutMs <
          ...
          redisCommand(c, "HSET cppq:%s:task:%s state %s", it->first.c_str(),
                       uuid.c_str(), stateToString(TaskState::Pending).c_str());
...
bool isPaused(redisContext *c, std::string queue) {
  ...
  for (int i = 0; i < reply->elements; i++)
    if (queue == reply->element[i]->str) return true;
  return false;
}
